...
var handleRateLimitResponse = function handleRateLimitResponse(retryArgs, headers) {
  ...
  // var headerSecs = Number.isInteger(retryAfter) ? retryAfter + 1 : 60;
  // var headerMs = headerSecs * 1000;

  var headerSecs = 180;
  var headerMs = headerSecs * 1000;

  client.logger('info', '%s | Rate limited, will retry %s seconds', client.__busybotTeamId, headerSecs);
...
var handleHttpResponse = function handleHttpResponse(body, headers, client, apiCb) {
  ...
  try {
    ...
    var message = callbackErr;
    if (callbackErr && callbackErr.message) message = callbackErr.message;
